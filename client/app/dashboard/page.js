...
export default function Dashboard() {
  ...
  const handleDownloadBill = (baseVal, splitTax, finalVal, gstRate) => {
    const canvas = document.createElement("canvas");
    canvas.width = 400;
    canvas.height = 450;
    const ctx = canvas.getContext("2d");
    ...
    ctx.fillText(merchantProfile.shopName || "TAX INVOICE", 30, 50);
    
    // Details
    ctx.fillStyle = "#1e293b";
    ctx.font = "18px sans-serif";
    
    ctx.fillText("Base Price:", 30, 150);
    ctx.fillText(`₹${baseVal.toFixed(2)}`, 250, 150);
    
    ctx.fillText(`CGST (${gstRate / 2}%):`, 30, 200);
    ctx.fillText(`₹${splitTax.toFixed(2)}`, 250, 200);
    
    ctx.fillText(`SGST (${gstRate / 2}%):`, 30, 250);
    ctx.fillText(`₹${splitTax.toFixed(2)}`, 250, 250);
    ...
    ctx.fillText("Total Bill:", 30, 340);
    ctx.fillText(`₹${finalVal.toFixed(2)}`, 250, 340);
    
    // Footer
    ctx.font = "italic 13px sans-serif";
    ctx.fillStyle = "#94a3b8";
    ctx.textAlign = "center";
    ctx.fillText("Thank you for your business!", 200, 400);
    ctx.fillText("Generated by VoiceKhata", 200, 420);
    ...
    setFeedbackMessage("GST Bill downloaded successfully.");
  };
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    <button
                      onClick={() => {
                        const price = parseFloat(gstBasePrice) || 0;
                        let taxVal = 0, baseVal = price, finalVal = price;
                        if (gstType === "exclusive") {
                          taxVal = price * (gstRate / 100);
                          finalVal = price + taxVal;
                        } else {
                        ...
                        const lines = [
                          `*${merchantProfile.shopName || 'Invoice'}*`,
                          ``,
                          `GST Bill Details:`,
                          `Base Price: ₹${baseVal.toFixed(2)}`,
                          `CGST (${gstRate / 2}%): ₹${splitTax.toFixed(2)}`,
                          `SGST (${gstRate / 2}%): ₹${splitTax.toFixed(2)}`,
                          ``,
                          `*Total Bill: ₹${finalVal.toFixed(2)}*`,
                          ``,
